...
class ConfigAutoLineageParams:
    ...
    @staticmethod
    def config_auto_lineage_retvals(key, value):
        if key == "busco_run" and value == "auto-lineage":
            return False
...
class TestConfigManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        def kill_patches():  # Create a cleanup callback that undoes our patches
            ...
            importlib.reload(ConfigManager)  # Reload our UUT module which restores the original decorator

        self.addCleanup(
            kill_patches)  # We want to make sure this is run so we do this in addCleanup instead of tearDown

        # Now patch the decorator where the decorator is being imported from
        patch('busco.BuscoLogger.LogDecorator', lambda
            *args, **kwargs: lambda f: f).start()  # The lambda makes our decorator into a pass-thru. Also, don't forget to call start()
        # HINT: if you're patching a decor with params use something like:
        # lambda *x, **y: lambda f: f
        importlib.reload(ConfigManager)  # Reloads the module which applies our patched decorator
    ...
    def test_get_config_from_env(self):
        ...
        with patch('os.access') as mockaccess:
            ...
            self.assertEqual(config_manager.config_file, os.environ.get("BUSCO_CONFIG_FILE"))

    def test_no_env_param_set(self):
        with self.assertRaises(SystemExit):
            ConfigManager.BuscoConfigManager({})

    @patch('__main__.ConfigManager_unittests.ConfigManager.logger', autospec=True)
    def test_config_validated(self, *args):
        config_manager = ConfigManager.BuscoConfigManager(self.params)
        with patch('__main__.ConfigManager_unittests.ConfigManager.BuscoConfigMain', autospec=True):
            config_manager.load_busco_config()
            config_manager.config.validate.assert_called()

    @patch('__main__.ConfigManager_unittests.ConfigManager.BuscoConfigMain', autospec=True)
    def test_log_warning_if_neither_lineage_nor_autolineage_specified(self, mock_config_main, *args):
        mock_config_main.return_value.check_lineage_present = lambda: False
        mock_config_main.return_value.getboolean.side_effect = [
            False, False, True, False, True] # These values comprise the three distinct logical paths into this part
        # of the code: run1: False, False; run2: True, (shortcircuit); run3: False, True.
        config_manager = ConfigManager.BuscoConfigManager(self.params)
        test_dataset_path = "/path/to/lineage_dataset"
        with patch.object(config_manager, "auto_select_lineage", lambda: test_dataset_path):
    ...
    @patch('__main__.ConfigManager_unittests.ConfigManager.BuscoConfigMain', autospec=True)
    def test_log_warning_if_both_lineage_and_autolineage_specified(self, mock_config_main, *args):
        mock_config_main.return_value.check_lineage_present = lambda: True
        mock_config_main.return_value.getboolean.side_effect = [True, False, True, False, False]
        config_manager = ConfigManager.BuscoConfigManager(self.params)
        test_dataset_path = "/path/to/lineage_dataset"
        with patch.object(config_manager, "auto_select_lineage", lambda: test_dataset_path):
            for _ in range(2):
    ...
    def test_config_updated_if_lineage_present(self, mock_config_main, *args):
        ...
        with patch.object(config_manager, "auto_select_lineage", lambda: test_dataset_path):
            config_manager.load_busco_config()
        calls = [call("busco_run", "auto-lineage", "False"),
    ...
    def test_update_dirname(self, mock_config_main, *args):
        ...
        with patch.object(config_manager, "auto_select_lineage", lambda: test_dataset_path):
            config_manager.load_busco_config()
        mock_config_main.return_value.set_results_dirname.assert_called_with(test_dataset_path)
    ...
    def test_lineage_downloaded(self, mock_config_main, *args):
        ...
        with patch.object(config_manager, "auto_select_lineage", lambda: test_dataset_path):
            config_manager.load_busco_config()
        mock_config_main.return_value.download_lineage_file.assert_called_with(test_dataset_path)
    ...
    def test_run_auto_select_if_no_lineage(self, mock_config_main, *args):
        ...
        mock_config_main.return_value.check_lineage_present = lambda: False
        test_dataset_path = "/path/to/lineage_dataset"
        with patch.object(config_manager, "auto_select_lineage", return_value=test_dataset_path):
            config_manager.load_busco_config()
            config_manager.auto_select_lineage.assert_called()


    @patch('__main__.ConfigManager_unittests.ConfigManager.AutoSelectLineage', autospec=True)
