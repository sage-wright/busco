    ...
    def get_gene_details(self):
        ...
        with open(self._output_fna, "rU") as f:
            for record in SeqIO.parse(f, "fasta"):
                ...
                self.gene_details[gene_name].append({"gene_start": gene_start, "gene_end": gene_end, "strand": strand})
                ...
                for m, match in enumerate(match_info):
                    ...
                    if self.mode == "proteins" or self.mode == "transcriptome":
                        ...
                        try:
                            ...
                            self.extra_columns = True
                            output_lines.append("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                                busco, label, gene_id, scaffold["gene_start"], scaffold["gene_end"], scaffold["strand"],
                                bit_score, match_length, link, desc))
                        except KeyError:
                            output_lines.append("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(
                                busco, label, gene_id, scaffold["gene_start"], scaffold["gene_end"], scaffold["strand"],
                                bit_score, match_length))
        return output_lines
    ...
    def _write_output_header(self, file_object, missing_list=False, no_table_header=False):
        ...
        if no_table_header:
            ...
            if self.extra_columns:
                ...
                file_object.write("# Busco id\tStatus\tSequence\tGene Start\tGene End\tStrand\tScore\tLength\n")
...
class MetaeukRunner(BaseRunner):
    ...
    ACCEPTED_PARAMETERS = ["comp-bias-corr", "add-self-matches", "seed-sub-mat", "s", "k", "k-score", "alph-size",
                           ...
                           "min-aln-len", "seq-id-mode", "alt-ali", "c", "cov-mode", "realign", "max-rejected",
                           ...
                           "search-type", "start-sens", "sens-steps", "metaeuk-eval", "metaeuk-tcov", "min-intron",
                           "min-exon-aa", "max-overlap", "set-gap-open", "set-gap-extend", "overlap", "protein",
                           "target-key", "reverse-fragments", "sub-mat", "db-load-mode", "force-reuse",
                           "remove-tmp-files", "filter-hits", "sort-results", "omit-consensus", "create-lookup",
                           "chain-alignments", "merge-query", "strand", "compressed", "v", "max-intron", "max-seq-len"]

    PRESET_PARAMETERS = ["--max-intron", "--max-seq-len", "--min-exon-aa", "--max-overlap", "--min-intron", "--overlap"]

    def __init__(self):
        super().__init__()
        self._output_folder = os.path.join(self.run_folder, "metaeuk_output")
        self._initial_results_folder = os.path.join(self._output_folder, "initial_results")
        ...
        self.param_keys = []
        self.param_values = []
        self.create_dirs([self._output_folder, self._initial_results_folder, self._rerun_results_folder])
        self.gene_details = None
        ...
        self.combined_pred_protein_seqs = os.path.join(self._output_folder, "combined_pred_proteins.fas")
    ...
    def configure_runner(self, incomplete_buscos=None):
        ...
        if self.run_number > 1:
            self._output_basename = os.path.join(self._rerun_results_folder, os.path.basename(self.input_file))
            self.refseq_db_rerun = os.path.join(self._output_folder, "refseq_db_rerun.faa")
            self._extract_incomplete_buscos_ancestral()
            ...
            self._output_basename = os.path.join(self._initial_results_folder, os.path.basename(self.input_file))
        ...
        try:
            if self.run_number == 1:
                ...
                self.extra_params = self.config.get("busco_run", "metaeuk_rerun_parameters").replace(',', ' ')
        except NoOptionError:
            self.extra_params = ""
        ...
        self.pred_protein_files.append(self.pred_protein_seqs_modified)
        self.bed_file = "{}.bed".format(self._output_basename)
    ...
    def combine_run_results(self):
        with open(self.combined_pred_protein_seqs, "w") as combined_output:
            for run_result in self.pred_protein_files:
                with open(run_result, "r") as f:
                    content = f.readlines()
                    combined_output.writelines(content)
        return
    ...
    def _extract_incomplete_buscos_ancestral(self):
        ...
        with open(self.refseq_db_rerun, "w") as out_file:  # Create new query file for second tblastn run
            SeqIO.write(matched_seqs, out_file, "fasta")
    ...
    def edit_file_header(self):
        ...
        with open(self.pred_protein_seqs, "rU") as f:
            for record in SeqIO.parse(f, "fasta"):
                ...
                self.sequences_aa[record.id] = record
                for i in range(int(header_details["num_exons"])):
                    bed_line = "{}\t{}\t{}\t{}\n".format(header_details["C_acc"],
                                                         header_details["all_taken_low_exon_coords"][i],
                                                         header_details["all_taken_high_exon_coords"][i],
                                                         header_details["S"])
        ...
        with open(self.bed_file, "w") as bedfile:
